...
# purpose: Functions on firing requests against ORS api
# on centrality and net graph capabilities
#
# 1 Libraries ---------------------------------
...
process_graph <- function(host="localhost", port=12345, aoi_bbox, aoi, profile, no_cores) {
...
targeted_centrality <- function(net, src.pts, dst.pts, no_cores) {
  #' Targeted centrality between a set of source and destination locations
  #'
  #' Parallel loop over all sources and destinations and aggregate the result
  #'
  #' @param network sfnetworks graph
  #' @param src.pts Set of source/origin locations
  #' @param dst.pts Set of destination locations
  ...
  tic("targeted centrality") # log main process

  # paralell run
  res <- mclapply(st_geometry(src.pts), function(src){


    src <- st_sfc(src, crs = st_crs(src.pts)) # ensure the source points are sfc and have the right crs

    temp_res <- lapply(st_geometry(dst.pts), function(dst){


      dst <- st_sfc(dst, crs = st_crs(src.pts)) # ensure the destination points are sfc and have the right crs
      ...
      paths_to_source <- net %>%
        ...
        pull(edge_paths) %>%
        unlist()
      paths_to_source_sf <- st_as_sf(slice(net, paths_to_source))
      paths_to_source_sf$count <- 1
      return(paths_to_source_sf)
    })
    temp_res_sf <- dplyr::bind_rows(temp_res)
    #message_parallel("done.")
  }, mc.cores=no_cores)
  ...
  # aggregate score by segment
  res_bc_direct <- res_sf  %>% dplyr::group_by(from, to, toId, fromId, weight, unidirectId, bidirectId, ) %>% summarize(centrality = sum(count),  .groups = "keep")

  return(res_bc_direct)
}
